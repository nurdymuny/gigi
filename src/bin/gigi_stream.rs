...
// `value_to_json` hoisted to gigi::wire (stream-extraction phase 2,
// family 3 — patterns::http::hunt_row_to_json consumes it; the rest of
// the wire-converter set follows with family 9; see EXTRACTION_MAP.md).
use gigi::wire::value_to_json;
...
async fn bundle_predict(
    State(state): State<Arc<StreamState>>,
    Path(name): Path<String>,
    Json(req): Json<SupervisedPredictRequest>,
) -> Result<Json<serde_json::Value>, (StatusCode, Json<ErrorResponse>)> {
    let engine = state.engine_read();
    ...
    let preds: Vec<serde_json::Value> = predictions.iter()
        .map(|(id, v)| serde_json::json!({ base.clone(): id, req.target.clone(): v })).collect();
    Ok(Json(serde_json::json!({
        "bundle": name,
        "target": req.target,
        "task": task,
...
/// POST /v1/bundles/{name}/reduce
///
/// PCA dimensionality reduction over the numeric fibers. Returns the principal
/// components (loadings), each record's reduced coordinates, the explained-variance
/// ratios, and the reconstruction error — the standard PCA deliverables.
async fn bundle_reduce(
    State(state): State<Arc<StreamState>>,
    Path(name): Path<String>,
    ...
    let rnd = |v: f64, p: f64| (v * p).round() / p;
    let comps: Vec<serde_json::Value> = components.iter().enumerate().map(|(i, comp)| {
        let loadings: serde_json::Map<String, serde_json::Value> = feature_names.iter().zip(comp)
            .map(|(f, w)| (f.clone(), serde_json::json!(rnd(*w, 100000.0)))).collect();
        serde_json::json!({ "pc": i + 1, "explained_variance_ratio": rnd(explained[i], 100000.0), "loadings": loadings })
    }).collect();
    let results: Vec<serde_json::Value> = (0..ids.len()).map(|i| {
        let rc: Vec<f64> = coords[i].iter().map(|v| rnd(*v, 100000.0)).collect();
        serde_json::json!({ base.clone(): ids[i], "coords": rc })
    }).collect();
    Ok(Json(serde_json::json!({
        "bundle": name,
        "n": ids.len(),
        "k": explained.len(),
        "components": comps,
        "cumulative_explained_variance": rnd(cumulative, 100000.0),
        "reconstruction_rmse": rnd(reconstruction_rmse, 100000.0),
        "variance_retained": rnd(variance_retained, 100000.0),
        "notes": notes,
        "results": results,
    })))
}

/// POST /v1/bundles/{name}/prescribe
///
/// Geometric diagnostic: reports the data's curvature, intrinsic-dimension ratio, and
/// spectral clarity, with plain-English reads and method suggestions. Describes the
/// shape; predicts no scores.
async fn bundle_prescribe(
    State(state): State<Arc<StreamState>>,
    Path(name): Path<String>,
    Json(req): Json<PrescribeRequest>,
) -> Result<Json<serde_json::Value>, (StatusCode, Json<ErrorResponse>)> {
    let engine = state.engine_read();
    let p = match prescribe_fingerprint(&engine, &name, req.neighbors, req.sample, &req.exclude) {
        Ok(v) => v,
        Err((code, msg)) => return Err((code, Json(ErrorResponse { error: msg }))),
    };
    Ok(Json(serde_json::json!({
        ...
        "fingerprint": {
            ...
            "dim_ratio": p.dim_ratio,
            "intrinsic_dim": p.intrinsic_dim,
            "ambient_dim": p.ambient_dim,
            "spectral_clarity": p.spectral_clarity,
        },
        "reads": p.reads,
...
/// POST /v1/bundles/{name}/solve
///
/// SVD fit store: one decomposition, then the whole ridge path with **exact** leave-one-out
/// — the model as a live geometric view of the bundle. Solves `Aθ = b` in the flat sector.
async fn bundle_solve(
    State(state): State<Arc<StreamState>>,
    Path(name): Path<String>,
    Json(req): Json<SolveRequest>,
) -> Result<Json<serde_json::Value>, (StatusCode, Json<ErrorResponse>)> {
    let engine = state.engine_read();
    let s = match fit_store_solve(&engine, &name, &req.target, &req.alphas, &req.exclude) {
        Ok(v) => v,
        Err((code, msg)) => return Err((code, Json(ErrorResponse { error: msg }))),
    };
    let round = |v: f64| (v * 100000.0).round() / 100000.0;
    let path: Vec<serde_json::Value> = s.path.iter().map(|p| serde_json::json!({
        "alpha": round(p.alpha), "loo_rmse": round(p.loo_rmse),
        "train_r2": round(p.train_r2), "effective_df": round(p.df),
    })).collect();
    let coefs: serde_json::Map<String, serde_json::Value> = s.coefficients.iter()
        .map(|(f, c)| (f.clone(), serde_json::json!(c))).collect();
    Ok(Json(serde_json::json!({
        "bundle": name,
        "n": s.n, "d": s.d, "rank": s.rank,
        "features": s.feature_names,
        "ridge_path": path,
        "best_alpha": round(s.best_alpha),
        "best_loo_rmse": round(s.best_loo_rmse),
        "coefficients": coefs,
        "reads": s.reads,
        "notes": s.notes,
    })))
...
/// POST /v1/bundles/{name}/circulation
///
...
async fn bundle_circulation(
    State(state): State<Arc<StreamState>>,
    Path(name): Path<String>,
    Json(req): Json<CirculationRequest>,
) -> Result<Json<serde_json::Value>, (StatusCode, Json<ErrorResponse>)> {
    let engine = state.engine_read();
    let c = match circulation_flow(&engine, &name, &req.source, &req.target, req.weight.clone(), req.charge, req.max_cycles) {
        Ok(v) => v,
        Err((code, msg)) => return Err((code, Json(ErrorResponse { error: msg }))),
    };
    let potential: Vec<serde_json::Value> = c.potential.iter()
        .map(|(node, score)| serde_json::json!({ "node": node, "potential": score })).collect();
    let cyclic: Vec<serde_json::Value> = c.cyclic_edges.iter()
        .map(|(s, t, w, r)| serde_json::json!({ "source": s, "target": t, "flow": w, "residual": r })).collect();
    Ok(Json(serde_json::json!({
        "bundle": name,
        "n_nodes": c.n_nodes,
        "n_edges": c.n_edges,
        "circulation_ratio": c.circulation_ratio,
        "magnetic_gap": c.magnetic_gap,
        "charge": c.charge,
        "ranking": potential,
        "cyclic_edges": cyclic,
        "reads": c.reads,
...
/// POST /v1/bundles/{name}/factorize
///
/// Recommender matrix factorization: learn rank-k latent factors + biases over the
/// observed (user, item, rating) triples and predict unobserved ratings. Reports
/// held-out RMSE vs the global-mean baseline; fills the rating for any records
/// where it is missing.
async fn bundle_factorize(
    State(state): State<Arc<StreamState>>,
    Path(name): Path<String>,
    Json(req): Json<FactorizeRequest>,
) -> Result<Json<serde_json::Value>, (StatusCode, Json<ErrorResponse>)> {
    let engine = state.engine_read();
    let FactorizeResult { base, rmse, baseline_rmse, rank, n_users, n_items, n_ratings, predictions, notes } =
        match factorize_matrix(&engine, &name, &req.user, &req.item, &req.rating,
            req.rank, req.epochs, req.reg, req.learning_rate) {
            Ok(v) => v,
            Err((code, msg)) => return Err((code, Json(ErrorResponse { error: msg }))),
        };
    let preds: Vec<serde_json::Value> = predictions.iter()
        .map(|(id, v)| serde_json::json!({ base.clone(): id, req.rating.clone(): v })).collect();
    Ok(Json(serde_json::json!({
        "bundle": name,
        "rank": rank,
        "n_users": n_users,
        ...
        "baseline_rmse": (baseline_rmse * 10000.0).round() / 10000.0,
        "predictions": preds,
        "notes": notes,
    })))
}

/// POST /v1/bundles/{name}/changepoints
///
/// Detect regime shifts in a time-ordered series via a sliding two-sample
/// t-statistic. Returns each changepoint's position, the field that shifted, and
/// the before/after means. Detection only — GIGI does not forecast.
async fn bundle_changepoints(
    State(state): State<Arc<StreamState>>,
    Path(name): Path<String>,
    Json(req): Json<ChangepointRequest>,
) -> Result<Json<serde_json::Value>, (StatusCode, Json<ErrorResponse>)> {
    let engine = state.engine_read();
    let ChangepointResult { base, n, window, threshold, time_field, value_fields, changepoints, notes } =
        match detect_changepoints(&engine, &name, &req.value, &req.time, req.window, req.threshold, req.max_changepoints) {
            Ok(v) => v,
            Err((code, msg)) => return Err((code, Json(ErrorResponse { error: msg }))),
        };
    let _ = base;
    let cps: Vec<serde_json::Value> = changepoints.iter().map(|(i, t, s, f, mb, ma)| serde_json::json!({
        "index": i, "time": t, "statistic": s, "field": f, "mean_before": mb, "mean_after": ma })).collect();
    Ok(Json(serde_json::json!({
        "bundle": name,
        "n": n,
        "window": window,
...
async fn ml_catalog() -> Json<serde_json::Value> {
    Json(serde_json::json!({
        ...
        "endpoints": [
            ...
            {"route": "POST /v1/bundles/{name}/prescribe", "kind": "geometric diagnostic (no prediction)",
             "does": "reads the data's geometric shape — curvature, intrinsic-dimension ratio, spectral clarity — and suggests which methods to TRY; makes no performance claim",
             "params": {"neighbors": "10", "sample": "500 (stride cap for O(n²) probes)", "exclude": "[] fibers to skip"}},
            {"route": "POST /v1/bundles/{name}/circulation", "kind": "geometric (rung two: directed flux / holonomy)",
             "does": "splits a directed flow into the ranking a global potential explains and the CIRCULATION no ranking can (loops/fraud rings); magnetic (Hermitian) Laplacian flux gap — the signal undirected diffusion is blind to",
...
// Patterns request/response structs + envelope_to_json / hunt_row_to_json
// moved to gigi::patterns::http (stream-extraction phase 2, family 3; see
// EXTRACTION_MAP.md). The four handlers below stay as thin wrappers:
// engine_write() + one call into the lib fn taking &mut Engine.
#[cfg(feature = "patterns")]
use gigi::patterns::http::{DefinePatternRequest, HuntRequest, PatternListEntry};
...
async fn list_patterns(
    ...
    let mut engine = state.engine_write();
    gigi::patterns::http::list_patterns(&mut engine)
}
...
async fn define_pattern_http(
    ...
    let mut engine = state.engine_write();
    gigi::patterns::http::define_pattern(&mut engine, req)
}
...
async fn drop_pattern_http(
    ...
    let mut engine = state.engine_write();
    gigi::patterns::http::drop_pattern(&mut engine, name)
}
...
async fn hunt_http(
    ...
    let mut engine = state.engine_write();
    gigi::patterns::http::hunt(&mut engine, bundle, req)
}
...
async fn main() {
    ...
    let app = app
        ...
        .route("/v1/bundles/{name}/cluster", post(bundle_cluster))
        .route("/v1/bundles/{name}/infer", post(bundle_predict))
...
mod tests {
    ...
    // The two `hunt_row_to_json_*` tests moved to gigi::patterns::http
    // with the helper they exercise (stream-extraction phase 2, family 3;
    // see EXTRACTION_MAP.md).
