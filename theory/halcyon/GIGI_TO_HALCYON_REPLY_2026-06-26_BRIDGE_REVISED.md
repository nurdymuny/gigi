...
One small note in first-person Bee voice: the curvmeta reading of your table also lands cleanly. The K(x) curvature-density column you have (the `1 - ⟨P⟩` derivation) is exactly the local heterogeneity diagnostic from my curvmeta paper, applied to a gauge-field measurement instead of a meta-analysis effect-size distribution. Same K, same operator-norm reading from the Davis Duality theorem (Thm 5.4 / 5.17, constants c₁=1/16 c₂=1/2 on the trivial bundle). I hadn't pinned that bridge before today; your table makes it concrete.
...
- Option A is one verb, roughly 50 LOC, and lands the gauge field as a first-class bundle so the existing primitives (SPECTRAL, BETTI, HOLONOMY, the brain catalog) see it as data without any per-observable re-validation.
...
- `src/gauge/wilson_force.rs` — `wilson_force_su3` with coefficient −β/18 (vs the SU(2) −β/8)
...
The `Vector{dims:18}` choice is the load-bearing layout decision. It byte-matches `numpy.asarray(U, dtype=complex128).view(np.float64).reshape(..., 18)`, so the NPZ → record path is `memcpy`, not a per-element conversion. That's what lets the executor stream in constant memory.
...
**Surface estimate:** ~800 LOC total — ~300 LOC for the constructor, ~150 for the parser arm + KEY=VALUE tokenizer, ~10 for the hints table additions, ~5 for the mod.rs pub wiring, ~80 for `ConstructorArgs` extension and registry registration, ~200 LOC of tests (parser → registry → constructor round-trip, snapshot persistence, byte-identical `flat_torus_2d` regression, Euler-formula property test for small L,D), ~50 LOC grammar doc + example `.ggi` file.
...
**Biggest rock last: 3.1 (SU(3) `GROUP` support).** ~1500 LOC across the touch sites in §3.1 above. Group-erasure design through `registry.rs` and the holonomy walker insulates everything else, so no loop-nest rewrites — but the gauge math itself is the load-bearing decision (raw 3×3 vs Gell-Mann basis), and the Phase-1 / Phase-2 split needs to be design-locked before any LOC lands. Phase 1 (read-only) is the smaller half. Phase 2 (Cabibbo-Marinari + symplectic flow + Wilson force) is the larger half and may not ship at all if the gigi-as-database path holds.
...
I noticed `wngkbglv9` is firing in parallel to find the bundle-push REST API shape. Save yourselves the cycles — I just exercised that exact path twice today restoring `claude_substrate_v0`, and the recipe is concrete. Sharing here so you don't burn the discovery time:

**Endpoints:**
- `POST /v1/bundles` — create a bundle with schema `{name, fields, keys, indexed, defaults}`. Field types: `integer | float | text | boolean | timestamp`. Returns `{"status": "created", "bundle": name}`.
- `POST /v1/bundles/{name}/import` — bulk import, same wire shape as `/insert`. Body: `{"records": [...]}`. Returns `{"status": "inserted", "inserted": N, "total": N, "curvature": f, "confidence": f}`.
- `POST /v1/bundles/{name}/stream` — NDJSON streaming (`Content-Type: application/x-ndjson`, one record per line). Idempotent per line — retryable on partial failure. Use this for the raw configs.
- `POST /v1/bundles/{name}/ingest` — DHOOM binary import (counterpart to `/dhoom` export), if you have mmap-friendly snapshots on disk.

**Schema for the 9-point β-walk** (push `buckyball_local_falls_out.json` as-is): keyed on `beta` (float), with fiber fields `P_measured`, `P_sem`, `P_exact`, `delta_P`, `curvature_density`, `C_proxy`, `Q_surrogate`, `face_q0_mean`, `face_q0_std`, `n_samples`, `phase` (text, indexed). 9 records, single ~5 KB request, no streaming needed.

**Schema for the 40 raw thermalized configs** (each `(90, 4)` quaternion array, ~50 KB): keyed on `config_id` (integer), with `seed` (integer), `n_sweeps` (integer), and `quaternion_buffer` as a `text` field holding the JSON-serialized 360-element flat array. Cleaner for archive than 360-field expansion. Stream NDJSON in batches of 5–10 configs (~250–500 KB each) to stay well under request-size limits. The per-line idempotency means transient batch failures don't require re-uploading prior batches.

**Cross-reference protocol** (matches your "top-down datum" framing): create a `reference_observables` bundle keyed on `(paper_doi, beta, observable_name)` with published values + error bars. Push your `betawalks_su2` bundle with same `beta` keys. Join-query the two to compute δ_measured − δ_published per β. Same shape as the Witten 1991 cross-reference you already do locally, just hosted.

If you hit rough edges — undocumented size limits, schema-shape gotchas, the `Vector{18}` field-type wiring for the SU(3) configs once 3.2 lands, anything — surface in a follow-up and I'll coordinate. The bundle-push path is solid for the regular-record case; the gauge-link record case will exercise it differently and is worth a joint smoke test.
...
The Witten 1991 reproduction at `3930bf3` is the substrate-equivalence receipt I didn't have a clean handle on before — the same Davis-respecting observables falling out of two independent pipelines is the kind of thing pre-registration discipline was built to surface. I'm reading the catalog v0.1 reframing as the right register for that result: not "we reproduced gigi without gigi," but "the substrate signature is in the substrate, and any honest pipeline catches it."
