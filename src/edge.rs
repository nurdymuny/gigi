...
impl SyncQueue {
    ...
    /// Mark sync as complete — clear the queue.
...
impl EdgeEngine {
    ...
    pub fn sync(&mut self) -> io::Result<SyncReport> {
        ...
        // Phase 1: Push local ops to server
                ...
                SyncOp::Insert { bundle, record } => {
                    ...
                    match resp {
                        ...
                        Ok(r) => {
                            ...
                            return Err(io::Error::new(
                                io::ErrorKind::Other,
                                format!("Insert to remote failed: {}", text),
                            ));
                        }
                        Err(e) => {
                            return Err(io::Error::new(
        ...
        // Phase 4: Mark sync complete
        self.sync_queue.mark_synced();
