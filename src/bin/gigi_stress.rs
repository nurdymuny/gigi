...
fn stress_convert() {
    ...
    metric("JSON chars:", &format!("{}", profile.json_chars));
    metric("DHOOM chars:", &format!("{}", profile.dhoom_chars));
    metric("Compression:", &format!("{:.1}%", profile.compression_pct));
    metric("Fields omitted:", &format!("{} / {} ({:.1}%)",
        profile.fields_omitted, profile.total_field_slots, profile.field_omission_pct));
    ...
    metric("Output size:", &format!("{} chars ({:.1} MB)", encoded.dhoom_chars,
