...
- **The durability encoder fix + the RIEMANN / Poincaré / P≠NP observable line + the Marcella EXPLAIN and dials family are LIVE (2026-07-16/17).** The DHOOM snapshot encoder's computed-field detection was `O(F³·N)` — days of compute on wide numeric bundles (384 scalar fibers), wedging the boot snapshot; it is now cached and capped at 64 candidate fields (`O(F·N + F³)`), so high-field bundles snapshot in seconds, with `.dhoom` + WAL formats byte-unchanged and old snapshots still opening. That is a durability fix for the boot-snapshot wedge only — heap-only bundles stay ephemeral until a whole-engine `/v1/admin/snapshot`. Three Millennium verbs restage geometric signatures Bee documents elsewhere as *gigi-native observables* — evidence inside the Davis framework, never proofs of the Clay problems: `SPECTRAL_GAUGE … MODE MAGNETIC [FULL [LIMIT k] | BULK k]` assembles a complex Hermitian magnetic Laplacian whose level-spacing reads in the GUE symmetry class (measured mean r̃ ≈ 0.605) against the real cos-weight Laplacian's GOE (≈ 0.527); `HOLONOMY … AROUND CYCLE` reads the lens-space π₁ = ℤ/p order off an SU(2) loop; `SPECTRAL … MODE MATRIX` exposes the raw signed-symmetric spectrum's negative fraction. Marcella-facing: `EXPLAIN SECTION … VECTOR / IN` (typed 404 on a missing key, additive `kappa_v` rows, mmap on-demand Welford stats), scoped `horizon` / `capacity` reads, and `POST …/windowed_coherence`. The dense eigensolver ceiling is opt-in via `GIGI_DENSE_CEIL` (default 4096, up to 8192); the sparse interior arm (Phase 2.1) merged and went live 2026-07-18 — `BULK` past the ceiling routes to it, `FULL` still returns the typed `SparseUnavailable`. See [`theory/halcyon/SPECTRAL_PHASE2_MAGNETIC_SHIPPED_2026-07-16.md`](theory/halcyon/SPECTRAL_PHASE2_MAGNETIC_SHIPPED_2026-07-16.md), [`HOLONOMY_CYCLE_SHIPPED_2026-07-17.md`](theory/halcyon/HOLONOMY_CYCLE_SHIPPED_2026-07-17.md), and [`MODE_MATRIX_SHIPPED_2026-07-17.md`](theory/halcyon/MODE_MATRIX_SHIPPED_2026-07-17.md).

- **TEXTURE + PRECEDENCE are LIVE (2026-08-09).** Two general-purpose verbs for "what shape is this data in", answered *without* resampling onto a grid whose bin width the caller guessed — a guess that is not free: on live asynchronous data, 3 of 6 channel pairs reported a different lead-lag under a time warp that preserved event order, and one reversed direction. `POST /v1/bundles/{name}/texture` returns the self-similarity (Hurst) exponent of one ordered numeric field plus an `r_squared` that tells the caller when the signal is *not* self-similar and `H` should not be read as a single number. `POST /v1/bundles/{name}/precedence` returns the normalised Lévy area between two fields — the level-2 antisymmetric part of the path signature — reading **record order and never a timestamp**, so there is no bin width to get wrong; `area > 0` means `x` leads `y`, and swapping the arguments negates it exactly. No domain vocabulary in either API (`field`, `x`, `y`, `order`): the shipped walkthrough measures queue latency, not markets. Sixteen blocking gates (TXP-1..15) cover planted-answer recovery, invariance to rescaling and to order-field spacing, six refusal paths that each name their reason, disclosure of skipped rows rather than coercion to 0.0, determinism, and record caps that error rather than return `null`. Suite **1001 passing**, 985 pre-existing untouched. Spec: [`theory/gigi/TDD-TXP_texture_precedence.md`](theory/gigi/TDD-TXP_texture_precedence.md); runnable example with planted answers and deliberate refusals: [`examples/texture_precedence_walkthrough.py`](examples/texture_precedence_walkthrough.py).

- **CADENCE is LIVE (2026-08-09).** The third shape verb, and the complement of the other two: TEXTURE and PRECEDENCE read *record order* and discard timestamps, while `POST /v1/bundles/{name}/cadence` reads exactly what they throw away — the spacing between events. It answers "is this stream arriving steadily or in bursts, and does the unevenness have memory?" for anything timestamped: log lines, sensor pings, request traces, seismic events, trades. It returns **two** numbers and always both, because one is not enough: `index` is the Greenwood dispersion statistic, whose null is exactly Dirichlet(1,…,1) at finite *n* **for every arrival rate**, so `E[index²] = 1` needs no simulation and does not move when the stream speeds up. But `R_n = Σt²/(Σt)²` is a **symmetric function of the gaps** — permuting them cannot change it — so `index` provably cannot see ordering, and a self-exciting stream reads identically to its own shuffle (measured: 1.448 vs 1.448). `memory`, the lag-1 autocorrelation of `ln τ`, is therefore an *exactly* non-overlapping second measurement rather than a correlated add-on. Verdict bands are **derived, not chosen**: `null_sd = n/√((n−1)(n+2)(n+3))`, bands at ±2 sd, checked against simulation from n = 64 to 8192. Eighteen blocking gates (CAD-1..18) include a gate that pins the order-blindness deliberately, a three-way separation of streams `index` alone would call identical, a refusal for a row index passed where a clock belongs (which would otherwise return a confident `index = 0` on a healthy feed), and a granularity refusal derived from a bias budget. Suite **1019 passing**, 1001 pre-existing untouched. Spec: [`theory/gigi/TDD-CAD_cadence.md`](theory/gigi/TDD-CAD_cadence.md); runnable example: [`examples/cadence_walkthrough.py`](examples/cadence_walkthrough.py).

- **Post-Kähler Phase 1 (PK-1..4) is LIVE (2026-07-25).** Four geometry verbs behind the `post_kahler_phase1` feature, each tied exactly to [`theory/post_kahler_directions/validation_tests.py`](theory/post_kahler_directions/validation_tests.py): `FISHER` (the information-geometry metric `g = diag(1/σ², 2/σ²)` per fiber, read free from L4 Welford variance), `WASSERSTEIN` (exact 1D `W₂` between two cohorts via the monotone rearrangement), `PERSISTENCE` (`H₀` via MST + elder rule → an *honest* cluster count that returns 1 when there is no real grouping), and `REEB` (the standard contact form `α = dz − y·dx` + Reeb-field verification). Each has a REST twin (`/v1/bundles/{name}/{fisher_metric, ml/wasserstein, topology/persistence, brain/reeb_flow}`). 18 module ground-truth tests + 3 endpoint/GQL integration tests; the default no-feature build stays byte-unchanged. Verbatim examples on R.A. Fisher's iris dataset in [`theory/reviews/iris_pk_results.json`](theory/reviews/iris_pk_results.json) (reproduce with [`scripts/pk_iris_examples.py`](scripts/pk_iris_examples.py)). Spec: [`GIGI_POST_KAHLER_SPEC_v0.1.md`](GIGI_POST_KAHLER_SPEC_v0.1.md).
...
completeness-proven vs dense ground truth for V ≤ 2048 (+ fresh-seed), merged and live 2026-07-18
(live probe V=4913: `mode_used="sparse_interior"`, converged 32/32, max_residual 2.528e-12,
30.2 s wall — [`theory/halcyon/SPARSE_INTERIOR_LIVE_2026-07-18.md`](theory/halcyon/SPARSE_INTERIOR_LIVE_2026-07-18.md)).
Its wall cost is O(V²) per solve — 739.69 s measured at V=8000, k=64; projected ~37 min at
V=13824 and ~3.4 h at V=32768 — inherent to the narrow-window/wide-band ratio and explicitly
out of scope to break.

The buried numeric validation artifacts — the Wilson-loop holonomy table on bilayer graphene,
Wu-Yang integrality, Berezin-Toeplitz semiclassics, the Poincaré-to-sharding TDD gates, the
encryption golden vectors — are indexed with verbatim numbers and source paths in
...
- **`GIGI_DENSE_CEIL`** — raises the dense eigensolver ceiling for `SPECTRAL_GAUGE … FULL` / `BULK` (the `MODE MAGNETIC` interior spectrum). Default 4096 vertices; an opt-in value is clamped to the safe band `[4096, 8192]` — raise-only, so it can never lower the floor or exceed 8192, because a V ≈ 8000 complex-Hermitian dense solve is ~2–3 GB peak RSS and will OOM a laptop. A `FULL` request above the ceiling returns a typed `SparseUnavailable` naming the memory cost and this knob; a `BULK` request above the ceiling routes to the sparse interior solver (Phase 2.1, live since 2026-07-18). A machine-safety knob, deliberately in the environment rather than the query.
...
-- Post-Kähler Phase 1 verbs (2026-07-25). Four geometry readouts gated on the
-- `post_kahler_phase1` feature; each ties exactly to
-- theory/post_kahler_directions/validation_tests.py. Ergonomic verb-first forms —
-- no ON clause defaults to every numeric fiber; GAP defaults to 2.
FISHER iris ON (petal_length, petal_width);            -- PK-2 Fisher metric g=diag(1/σ²,2/σ²) per fiber, from L4 Welford variance
PERSISTENCE iris ON (petal_length, petal_width) GAP 2;  -- PK-4 H₀ persistence (MST + elder rule) → honest cluster count
WASSERSTEIN iris ON petal_length BETWEEN species = 0 AND 2;  -- PK-3 exact 1D W₂ between two cohorts (monotone rearrangement)
REEB iris ON (petal_length, petal_width, sepal_length);      -- PK-1 standard contact form α=dz−y·dx + Reeb-field verification
...
for the public deterministic invariant check, the Halcyon read
routes (`/v1/lattice/*`, `/v1/gauge_field/*`, `/v1/e_field/{name}`,
`/v1/symplectic_flow/diagnostics/:run_id`), and the Post-Kähler Phase 1
routes (`/v1/bundles/{name}/fisher_metric`, `.../topology/persistence`,
`.../ml/wasserstein`, `.../brain/reeb_flow` — each the REST twin of its
GQL verb), and the shape verbs `/v1/bundles/{name}/texture` (self-similarity
