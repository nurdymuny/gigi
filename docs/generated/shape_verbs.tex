...
\texttt{ON} names the field or fields being measured --- the same \texttt{ON} you already use on \texttt{CURVATURE btc\_l2 ON mid}.
...
**CADENCE takes no \texttt{ALONG}** --- \texttt{ON} there is the clock itself, and its values are the whole measurement, so there is nothing to re-order it by. Writing one is an error that says so rather than being quietly ignored.
...
Same three verbs at \texttt{/v1/bundles/\{name\}/\{texture,precedence,cadence\}}. Use this when you want the full response --- REST additionally returns the \texttt{reads} array and the \texttt{notes} array, which GQL's row envelope does not carry.
...
| | \texttt{area} | |---|---| | ordered \texttt{ALONG seq} | \textbf{+0.7536} | | \texttt{order} omitted | \textbf{+0.0017} |
...
Every response carries a **\texttt{reads}** array: plain-English sentences interpreting the numbers. You should not need this document open to act on a result. If a \texttt{reads} line ever contradicts what you expected, trust it over your assumption --- it is generated from the same numbers the verdict is.
...
\item \texttt{H > $\tfrac12$} --- \textbf{persistent.} Moves tend to continue. A move is evidence of more of
\end{itemize}
...
\item \texttt{H < $\tfrac12$} --- \textbf{anti-persistent.} Moves tend to reverse. Choppy; trend-following
\end{itemize}
...
The old flat $\pm$0.05 was \emph{narrower than the estimator's own noise} for any series under about 16,000 points, so a true random walk was being called ROUGH or SMOOTH roughly a third of the time. The band now widens on short series instead of claiming a precision the fit does not have. \texttt{h\_sd} comes from an empirical law fitted to simulation (\texttt{0.29$\cdot$n\textasciicircum{}$-$0.267}), not a closed form --- stated plainly so you know what it rests on.
...
\midrule
\textbf{> 0} & **\texttt{x} leads \texttt{y}** --- x moves first, y follows \\
\textbf{< 0} & **\texttt{y} leads \texttt{x}** \\
...
| increments | TEXTURE reads | true noise floor | |---|---|---| | strongly mean-reverting | ROUGH | \textbf{0.006} | | random walk | RANDOM\_WALK | 0.458 | | strongly trending | SMOOTH | \textbf{1.046} |

A factor of 160. So the response now carries a **\texttt{p\_value}\textbf{ from a rotation test built on \emph{your} data: one channel's increments are circularly rotated, which destroys any lead relationship while preserving that channel's own autocorrelation exactly, and the observed area is ranked against 199 such surrogates. Measured false-positive rate across that whole roughness range: }4.3\% / 4.0\% / 3.3\% / 6.3\% / 6.7\%** against a nominal 5\%.

| field | meaning | |---|---| | \texttt{p\_value} | probability of an area this large with no lead relationship | | \texttt{null\_sd} | the noise floor for \emph{this} pair --- expect it to vary a lot | | \texttt{significant} | \texttt{p\_value <= 0.05} | | \texttt{null\_n} | records the null was built from |

**\texttt{significant: false} does not mean "no lead."** It means this window cannot establish one. That is the common case, and it is not a defect --- it is what honest lead-lag measurement looks like on a single window.
...
**\texttt{index} --- how uneven the spacing is.**
...
**\texttt{memory} --- whether the unevenness persists.**
...
**\texttt{index\_blocked} is \texttt{null} when there are fewer than two blocks of data** (under \textasciitilde{}128 gaps at the default block of 64). It is deliberately absent rather than a placeholder \texttt{0.0}, which would read as "no structure at any scale" instead of "not enough data to look". \texttt{n\_blocks} is 0 in that case.
...
**\texttt{index} below 1 is a feed-quality alarm.** Real sources are not more regular than random. If you see \texttt{THROTTLED}, something is metering the stream between the source and you --- a poll interval, a rate limiter, a fixed flush --- and every number computed downstream is describing your own plumbing rather than the market.
...
\item \textbf{CADENCE first.} If it reads \texttt{THROTTLED}, stop --- you are measuring your own
\end{enumerate}
...
\item \textbf{PRECEDENCE last}, once you trust the feed and know the regime, to ask what
\end{enumerate}

is actually leading --- and \textbf{across several windows, not one}. Check \texttt{significant}; if it is false, that window cannot establish a lead. Collect the signed \texttt{area} per window and test the mean against zero. One window detects a genuine lead only about 20\% of the time; twenty separate at z = 14.5.
...
Rows with missing or non-finite values are **skipped and counted in \texttt{notes}**, never coerced to zero. Check \texttt{notes} and \texttt{n} if a result surprises you --- you may be measuring fewer records than you loaded.
...
REST & \texttt{POST /v1/bundles/\{name\}/texture} & \texttt{.../precedence} & \texttt{.../cadence} \\
...
Runnable examples with planted answers and deliberate refusals: [\texttt{examples/texture\_precedence\_walkthrough.py}](../examples/texture\_precedence\_walkthrough.py) $\cdot$ [\texttt{examples/cadence\_walkthrough.py}](../examples/cadence\_walkthrough.py)
