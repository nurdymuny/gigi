...
| --- | --- | --- |
| `substrate_wall < 25 ms` (warn), `< 50 ms` (fail) | Sprint A baseline is live (7 ms substrate + ~10-30 ms RTT) | A pre-Sprint-A binary (the old ~25 ms baseline before the `face_edges` hoist), or network congestion. The probe times the `GIBBS_SAMPLE` round-trip from the client — network RTT adds ~10-30 ms over the substrate cost at this scale. |
| `MeanPlaquette[199] == 0.5125429110231062` | bit-identity at the chain endpoint scalar | RNG drift, sweep-count off-by-one, β decode bug, or a measurement-history index shift. This is the cheaper of the two state receipts; if it passes but the SHA fails, the divergence is somewhere outside the sweep-199 scalar. |
