...
impl GaugeKey {
    ...
    pub fn random_seed() -> [u8; 32] {
        // Use system time + address entropy for non-crypto randomness
        // (In production, use a CSPRNG — but this avoids adding a dependency)
        let mut seed = [0u8; 32];
        ...
        // Mix in a pointer address for additional entropy
        let stack_var = 42u64;
        let addr = &stack_var as *const u64 as u64;
        for i in 0..8 {
            seed[16 + i] = ((addr >> (i * 8)) & 0xFF) as u8;
        }
        // Final mix
        let h = Self::mix_hash(&seed, 0xdeadbeefcafe1234);
        for i in 0..8 {
            seed[24 + i] = ((h >> (i * 8)) & 0xFF) as u8;
        }
        seed
    }
}
