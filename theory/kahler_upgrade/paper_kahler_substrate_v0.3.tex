...
%% Status: DRAFT v0.3 — §§1–4 fully written; §§5–9 as detailed key beats.
%%         Severity-1 fixes from the adversarial review applied:
%%           S1.1 Definition 1.1 connection ambiguity → Levi-Civita w/ footnote
%%           S1.2 Theorem 5.1 → Proposition + Empirical Observation
%%           S1.3 Theorem 6.1 → Proposition (Operational equivalence)
%%           S1.4 "four decimal places" → precise rel_err + integrality dev
%%           S1.5 Wu-Yang range comparison → honest "comparable, slightly above"
...
\normalsize \emph{Draft v0.3 --- \S\S1--4 full text; \S\S5--9 key beats; \\
post-adversarial-review (Severity~1 fixes applied)}}
...
Under a reproducible-build configuration of the GIGI Rust codebase
(deterministic compiler flags, no source-path-dependent hashing, no
embedded timestamps), the no-feature build (\texttt{cargo build}
without \texttt{--features kahler}) produces a byte-identical engine
binary independent of which of the eight K\"ahler layer commits is
checked out of the repository, provided all 720 no-feature tests
pass.
\end{proposition}

\paragraph{Verification (engineering).}
...
time. Under reproducible-build configuration, the
\texttt{libgigi.rlib} output is determined entirely by the
non-K\"ahler source code, which is unchanged across the eight layer
commits.
