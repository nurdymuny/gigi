...
fn main() {
        ...
        "csv" => {
            ...
            match result {
                Ok(enc) => {
                    ...
                    eprintln!("Encoded {} chars → {} chars ({:.1}% compression)",
                        enc.json_chars, enc.dhoom_chars, enc.compression_pct);
                    return;
                }
                Err(e) => {
    ...
    eprintln!("Encoded {} records | {} → {} chars | {:.1}% compression",
        json_array.len(), result.json_chars, result.dhoom_chars, result.compression_pct);
}
