...
///
/// The 13 variants this matches are exactly the set the spec §2.5
...
/// follow-up Part V verb does not re-hit the same drop bug).
///
...
///
/// Hand-off contract: the engine handle is only mutated by the
/// `GaugeField { persist: true, .. }` arm (durable PERSIST routing
/// through `engine.declare_gauge_field_durable`). Every other
/// arm operates on the process-global registries. The caller passes
/// the `Arc<RwLock<Engine>>` through unchanged so the existing
/// `StreamState::engine` field works without any signature change.
pub fn try_dispatch_gauge_statement(
    ...
    // Acquire a write lock so the (rare) PERSIST path can route
    // through `engine.declare_gauge_field_durable`. The non-PERSIST
    // arms touch process-global registries and ignore the engine;
    // the cost of holding the write lock during a registry-only
    // statement is a few microseconds at most — acceptable for a
    // P0 fix that closes a real correctness bug.
