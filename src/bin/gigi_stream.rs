...
async fn rate_limit_middleware(
    ...
    // Extract IP from connection info or fallback
    let ip = req.headers()
        .get("x-forwarded-for")
...
async fn consistency_check(
    ...
    // Čech cohomology H¹ — check holonomy around all triangles
    // H¹ = 0 means fully consistent
    let k = curvature::scalar_curvature(store);
    // For a well-formed bundle, H¹ should be 0
    let h1 = if k < 1e-6 { 0 } else { 0 }; // simplified — real H¹ needs cycle detection
...
async fn export_dhoom(
    ...
    Ok(Json(serde_json::json!({
        ...
        "dhoom": result.dhoom,
        "json_chars": result.json_chars,
        "dhoom_chars": result.dhoom_chars,
        "compression_pct": result.compression_pct,
        "fields_omitted": result.fields_omitted,
        "total_field_slots": result.total_field_slots,
    })))
}
...
async fn gql_query(
    ...
    match &stmt {
        ...
        // v2.1: statements that don't need a bundle
