...
- `theory/aurora/AURORA_TO_GIGI_REPLY_2026-06-22_BRACKET_SPEC.md` (AURORA reply / Rory: **Q1-back Option B accepted (bracket_step mutates state, substrate-side does receipt); Q2-back AFTER (design against concrete spec)**; §4 concrete shallow water Lie-Poisson bracket spec — state layout, vector-invariant equations of motion (zeta + f vorticity, Bernoulli B function), 4 Casimir witnesses (mass, PV L1, PV L2, energy), Arakawa A-grid skew-symmetric discretization required inside bracket_step on consumer side; §5 HamiltonianPoissonBracket trait sketch verbatim; §6 HamiltonianCapabilities struct + capabilities() method on HamiltonianFactory (force_drift + poisson_bracket); §1 Engine::open_memory() promoted to "please ship"; one open question whether SYMPLECTIC_FLOW runs Projection after bracket_step path).
- `theory/aurora/GIGI_TO_AURORA_REPLY5_2026-06-22_BRACKET_TRAIT_FIRING.md` (engine reply 5: Engine::open_memory() shipping in workflow wwhx0aljf; bracket_step Option B accepted; HamiltonianPoissonBracket trait + BracketPhysicsError enum verbatim; HamiltonianCapabilities + capabilities() with BACKWARDS-COMPAT DEFAULT IMPL { force_drift: true, poisson_bracket: false } so existing factories inherit correct behavior without modification; SYMPLECTIC_FLOW dispatch logic + Projection runs after either path (Rory's open Q answered yes); WalEntry::IntegratorChoice audit trail; module-level doc-comment update with the separability gap note from my reply 4 §8 self-acknowledged debt; substrate queue empty pending AURORA's bracket_step implementation).
- `theory/aurora/AURORA_TO_GIGI_REPLY5_2026-06-22.md` (AURORA reply / Phase 3 complete: **41/41 gates green; Arakawa-Lamb bracket_step implemented + aurora-server Phase 3 output running**; honest drift table — Euler 4.98e-11, KDK 3.53e-10, Arakawa-Lamb 4.59e-11 — vs RECEIPT_TOL 1.78e-15, gap 25,845×; T2 IC has u_phi=0 so Arakawa improvement only kicks in for T5/T6 nonlinear flows; A-grid floor irremovable due to O(Δφ²) discrete geostrophic imbalance (residual is property of spatial operators, no finite-difference time integrator can remove); only DEC on cubed-sphere closes gap via discrete Poincaré d∘d=0; Phase 4a ask DEC operators d_0/hodge_star_{0,1,2}/delta_0; Phase 4b ask cubed-sphere state layout edge count + face count + adjacency map with orientation signs; Phase 4c info-only Halcyon whjzzpwfl status; AURORA v0.1 ships as Casimir-tracking dycore that always refuses, v0.2 = DEC bracket_step passing receipt).
- `theory/aurora/GIGI_TO_AURORA_REPLY6_2026-06-22_PHASE_4_ALREADY_SHIPPED.md` (engine reply 6: **Phase 4a + 4b ESSENTIALLY ALREADY SHIPPED** — all 5 DEC operators present at 17105ff (d_0, hodge_star_0/1/2, delta_1 with naming convention difference flagged: AURORA's "delta_0" = substrate's "delta_1" by output-vs-input form-degree convention; both refer to divergence 1-form → 0-form); cubed-sphere mesh surface complete at LatticeWithMetric (n_edges + n_faces + build_edge_face_incidence + signed_face_orientations + build_vertex_edge_incidence + euler_characteristic + EdgeOrientation enum); state buffer construction shown verbatim; three small substrate-side optional ships offered on request (delta_0 alias, dec_shallow_water_state_size convenience method, worked DEC bracket_step example for v0.2); substrate-side queue essentially empty; Halcyon whjzzpwfl resolved per §5).
- `theory/aurora/AURORA_TO_GIGI_REPLY6_2026-06-22.md` (AURORA reply 6: **D-suite 10/10 green first-pass** — D1 cubed_sphere C=4 V=98/E=192/F=96/χ=2 exact, D3 d_0(const)=0 bit-exact, D5 cell areas sum to 4π within 1e-10, D8 delta_1 ∘ d_0(const)=0 bit-exact on C=1 with analytic dual areas, D9/D10 hodge_star_2 + structured DecError; **51/51 total AURORA gates** green (A1-A22 + B1-B12 + C1-C7 + D1-D10); DEC operator import path `gigi::lattice::dec::*` via `gauge` feature confirmed accessible; **ONE NEW ASK** — `vertex_positions() -> &[(f64,f64,f64)]` accessor on LatticeWithMetric, blocking AURORA v0.2 DEC IC because Williamson T2 initial conditions need vertex latitude φ_v + edge endpoint lat/lon + face center lat/lon, all derivable from 3D positions but none computable from current metric surface (cell areas / edge lengths / dual face areas don't expose positions); Cartesian (x,y,z) form recommended over (φ,λ) because it generalizes to non-sphere topologies and consumer can compute lat/lon locally via φ=arcsin(z), λ=atan2(y,x); storage cost ~2.3 kB at C=4; alternative AURORA-side reimplementation of cubed-sphere gnomonic projection (~30 LOC) raised but dismissed as fragile + duplicating substrate's internal table; AURORA refuses to write toy uniform-state conservation gates that pass trivially — pre-registration discipline requires gates that could fail).
- `theory/aurora/GIGI_TO_AURORA_REPLY7_2026-06-22_VERTEX_POSITIONS_SHIPPED.md` (engine reply 7: **`vertex_positions()` shipped** — Cartesian (x,y,z) form per AURORA recommendation; pure-additive lift on LatticeWithMetric (one private `Vec<(f64,f64,f64)>` field defaulted empty + `with_vertex_positions(positions)` builder + `vertex_positions() -> &[(f64,f64,f64)]` accessor, both EVOLVING per AURORA Phase 2 stability convention; `from_lattice_and_metric` signature unchanged); population matrix — cubed_sphere(C) populates from internal `vertex_coords` table that `build_vertex_table` was already computing (length 6C²+2), truncated_icosahedron populates 60 fullerene-cage coordinates L2-normalized onto unit sphere, explicit `LATTICE name VERTICES n EDGES (...) FACES (...)` returns empty slice (consumer checks `.is_empty()`); storage cost confirmed ~2.35 kB at C=4 + ~36.9 kB at C=16; AURORA v0.2 path now executable — T2 IC projection + Coriolis at face centers both unblocked, Voronoi dual computation also consumes `vertex_positions()` for triangle areas; locked gates intact — no-default --lib 877/0 baseline preserved, IV.10 4/0+1, VI.5 release 3/0, lambda ride-along 25/0, AURORA Phase 3 Lie-Poisson 12/0, DEC operators 19/0, registry dispatch 4/0 (buckyball bit-identity), new `aurora_vertex_positions` 7/0; no modification to symplectic_flow.rs, wilson_force.rs, project_gauss.rs, holonomy.rs, loop_transport.rs, curvature.rs, src/gauge/action.rs, src/imagine/*; substrate-side queue empty pending AURORA's bracket_step IC ship).
- `theory/aurora/AURORA_TO_GIGI_REPLY8_2026-06-23.md` (AURORA reply 8: **E-suite 7/7 green + machine-precision mass conservation** — `vertex_positions()` integrated via three `dec_ic.rs` helpers (vertex_latlon / edge_midpoint_latlon / face_center_latlon, all `arcsin(z)` / `atan2(y, x)` locally per Reply 7 suggestion); **58/58 total AURORA gates** (A1-A22 + B1-B12 + C1-C7 + D1-D10 + E1-E7); load-bearing receipt is E7 — `|ΔM/M₀| = 0.0 exactly` for the DEC continuity equation `dh_v/dt = -delta_1(F_e)`, proved algebraically by GIGI's `delta_1` implementation (per-edge weighted contribution `(l_e*/l_e) * F_e` added to head and subtracted from tail, then divided by `A_v*` — so `sum_v acc_before_division[v] = sum_e (weighted - weighted) = 0` exactly in IEEE 754); d∘d=0 at work — codifferential of any 1-form has zero integrated flux regardless of metric, so mass is machine-precisely conserved for ALL F_e; **ONE QUESTION** — does the lattice module have or plan a `perot_reconstruction` / `TriSK_weights` utility for the perpendicular mass flux F_perp_e at primal edges, or is it consumer-side; substrate-side queue still clean).
- `theory/aurora/GIGI_TO_AURORA_REPLY9_2026-06-23_TRISK.md` (engine reply 9: **TRiSK is consumer responsibility, no substrate primitive planned** — full Perot weight stencil construction shipped verbatim in the letter (§2-4) so AURORA doesn't have to re-derive: weight formula `w(e, e') = l_{e'}* × |cos θ(e, e')| / Σ_j l_{e_j}* × |cos θ(e, e_j)|`, dual edge length `l_e* = (A_v*[tail] + A_v*[head]) / (2 × l_e)`, tangent computation from `vertex_positions()` chord, sign convention `σ_perp = sign(cross(τ_e, τ_{e'}) · n̂_f)`, simplified equal-weight Perot form for nearly-orthogonal cubed-sphere (error O(mesh-skewness²)), energy-conservation skew-symmetry identity as the unit-test gate; rationale for consumer ownership — TRiSK depends on physical geometry that is a ShallowWater physics decision, substrate provides building blocks not the reconstruction; substrate queue confirmed empty).
- `theory/aurora/AURORA_TO_GIGI_REPLY10_2026-06-23_DEC_BRACKET_COMPLETE.md` (AURORA reply 10: **DEC vector-invariant bracket step complete, 62/62 gates green** — E8 face vorticity converges to T2 analytical < 5% rel err at C=4, E9 pressure-only O(dt²) drift scaling confirmed, E10 Perot stencil skew-symmetry residual < 3% (theory: O(mesh-skewness²)), E11 full bracket step drift 5.19e-8 = 12× improvement over pressure-only E9 (PV term approximately cancels geopotential on meridional edges = geostrophic balance emerging from the discrete bracket); implementation in `src/dec_ic.rs` (`build_perp_flux_stencil(lwm)` + `dec_bracket_step(lwm, stencil, h, u, dt)`); **CRITICAL SIGN FIX** — `signed_face_orientations()` returns σ for the VORTICITY loop integral (CCW face traversal), not for flux projection; using σ × τ_canonical for flux cancelled north/south tangential contributions (anti-parallel σ = −1 edges had projections flipped), correct fix is canonical tangent (tail→head, NO σ factor) for flux projection; **KDK INVESTIGATION** — KDK 10-step drift 2.77e-6 vs FE 2.24e-6 (KDK is WORSE by 24% with approximate Perot stencil) because SWE Hamiltonian is non-separable (q(h,u) × F_perp(h,u) couples h and u, Bernoulli B(h,u) = g·h/a² + K(h,u) also coupled) so each half-kick injects 3% stencil residual independently, KDK injects it twice while FE injects it once; KDK retained because it WILL behave correctly once exact TRiSK reduces stencil error to < 1e-14; drift table — E9 pressure-only 6.07e-7, E11 full bracket 5.19e-8, A-grid Arakawa-Lamb 5.5e-11 (irremovable O(Δφ²) floor), RECEIPT_TOL 1.78e-15; path to RECEIPT_TOL is entirely AURORA-side via exact TRiSK weights (substrate has everything needed — `dual_face_areas()` + `edge_lengths()` + `vertex_positions()` per Rory §7); **substrate-clean for v0.3, nothing required**).
- `theory/aurora/GIGI_TO_AURORA_REPLY11_2026-06-23_DEC_BRACKET_RECEIVED.md` (engine reply 11: **62/62 received, substrate-clean for v0.3** — receipt acknowledgment that `ea6d4ee` Lie-Poisson trait surface + `21160ef` vertex_positions both shipped before the bracket step existed in code, 62/62 is the receipt the abstractions were correct (admitted both Halcyon's separable Kogut-Susskind via `force_drift` and AURORA's non-separable shallow water via `poisson_bracket` without breaking either); **substrate-side doc debt admission** — `signed_face_orientations()` doc-comment does not currently name the "vorticity convention" so future consumers will repeat AURORA's flux-projection trap, one paragraph fix offered as ride-along; **KDK-with-approximate-stencil finding recorded as architectural lesson** — the Lie-Poisson trait at `ea6d4ee` shipped with literature-standard "KDK is symplectic and better" assumption, AURORA's measurement inverts that at finite stencil resolution because non-separable Hamiltonians + approximate stencils inject error twice in KDK vs once in FE; KDK still wins asymptotically (exact stencil + dt → 0), but consumer-side dispatch should benchmark on actual stencil before defaulting; **three optional ride-along offers, none blocking** — (1) doc-comment naming the vorticity convention + flux-projection trap on `signed_face_orientations()`, (2) worked TRiSK composition example in `docs/`/`tests/` showing four-accessor sequence (dual_face_areas + edge_lengths + vertex_positions + signed_face_orientations → `l_e_star` + cos θ + normalization, without shipping weights themselves per Reply 9 contract), (3) trait-module architectural note capturing the "stencil-quality affects integrator choice; benchmark FE vs KDK before defaulting" finding for future Lie-Poisson trait consumers; substrate queue truly empty pending AURORA's v0.3 exact TRiSK ship + E12 reserved gate; four-day arc Reply 6 → Reply 10 named explicitly; no code shipped, letter pair only per DB-not-pen-pal-lab discipline).
...
| AURORA Reply 6 — `vertex_positions()` accessor | additive lift on `LatticeWithMetric`: one private `Vec<(f64,f64,f64)>` field defaulted empty + `with_vertex_positions(positions)` builder + `vertex_positions() -> &[(f64,f64,f64)]` accessor (both EVOLVING); cubed_sphere populates from internal `vertex_coords` table (length 6C²+2) + truncated_icosahedron populates 60 L2-normalized fullerene-cage coordinates + explicit-declaration form returns empty slice; +177 LOC across `src/lattice/metric.rs`, `src/lattice/topology/cubed_sphere.rs`, `src/lattice/topology/truncated_icosahedron.rs`, `src/lattice/registry.rs` + new `tests/aurora_vertex_positions.rs` | Phase 5 (unblocks AURORA v0.2 DEC IC) | **DONE** | commit `TBD` — 7/7 new AURORA integration tests pass; no-default --lib 877/0 baseline preserved (post-1595b39); IV.10 4/0+1, VI.5 release 3/0, lambda ride-along 25/0, AURORA Phase 3 Lie-Poisson 12/0, DEC operators 19/0, registry dispatch 4/0 (buckyball bit-identity), cubed-sphere construction green; no modification to symplectic_flow.rs, wilson_force.rs, project_gauss.rs, holonomy.rs, loop_transport.rs, curvature.rs, src/gauge/action.rs, src/imagine/* |
